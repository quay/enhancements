...
At the moment, Quay operator users have no ability to override the size of the PVCs for managed Postgres and Clair (Postgres) components. They are, however, able to customize the volume size for ObjectStorage when setting up Noobaa. This proposal would allow users to set volume sizes for all three of these components from the CR.
...
- Volume Size Overrides for unmanaged components
